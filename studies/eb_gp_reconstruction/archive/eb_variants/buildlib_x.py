...
from config import (DENSE_GRID_A, FEM_LADDER, EB_REPO, EB_GEOMETRY,
                    OUT, FIG, add_eb_to_path)
add_eb_to_path()
...
FLIB=np.linspace(185e3,1350e3,3200)    # library frequency axis (= Omega*F_REF)
...
np.savez_compressed('out/eblib_x.npz',K1=K1,GG=GG,FLIB=FLIB,F_REF=F_REF,
                    x_model=x_model,x_meas=x_meas,eta=eta,logamp=LA,k_lever=P.k_static)
print('saved out/eblib_x.npz  logamp',LA.shape,f'{LA.nbytes/1e6:.0f} MB raw')
