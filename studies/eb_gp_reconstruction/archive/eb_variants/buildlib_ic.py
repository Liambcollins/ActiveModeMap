...
for ik,k1 in enumerate(K1):
    for ig,g in enumerate(GG):
        cfg=EB.EBConfig(probe=P,model='2seg',phi=np.deg2rad(11.),k1=float(k1),k2=float(k1),k_cone_lat=100.,
...
np.savez_compressed(str(OUT) + '/eblib_ic.npz',K1=K1,GG=GG,FLIB=FLIB,F_REF=F_REF,
                    x_model=x_model,x_meas=x_meas,eta=eta,logamp=LA,k_lever=P.k_static)
print('saved out/eblib_ic.npz  logamp',LA.shape,f'{LA.nbytes/1e6:.0f} MB raw')
